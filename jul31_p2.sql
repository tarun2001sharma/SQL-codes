...
WITH FilteredCSM AS (
  ...
  pd.USERS_LICENSED,
    ...
    WHERE 
        ...
        AND PD.PLAN_ARR >= 5000 -- Ensure this filter is correctly applied here
),
...
//        NEXUS.BASE.PLAN BP
//    JOIN 
//        NEXUS.BASE.PLAN_DAILY pd ON BP.PLAN_UID = pd.PLAN_UID 
//    WHERE
//        pd.DATE = '2024-07-01'
//        AND BP.ARR >= 5000 
//        AND pd.PLAN_ARR >= 5000
//        AND BP.ACCOUNT_ID IN ('0014000000uWpyPAAS')
//),
//
...
//        SPLIT_PART(FWA.OWNER_PLAN_UID, '.', 3) AS WorkAPP_PlanUID,
...
//        COUNT(DISTINCT CASE WHEN FSM.DATE >= DATEADD(DAY, -90, CURRENT_DATE()) THEN FSM.SOLUTION_UID END) AS solutions_created_last_90_days,
        COUNT(CASE WHEN FSM.QUARTERS_ACTIVE = 1 THEN FSM.SOLUTION_UID END) AS solutions_created_last_90_days,
//        COUNT(DISTINCT CASE WHEN FSM.DEPARTMENT IS NOT NULL THEN FSM.DEPARTMENT END) AS num_departments_per_solution,
...
//        COUNT(DISTINCT FWA.CONTAINER_UID) AS work_app_count,
//        COUNT(DISTINCT FWA.OWNER_USER_UID) AS external_views_with_work_app,
        MAX(AMS.mobile_users_count) AS mobile_users_count,
        COALESCE(MAX(RM.rm_plan), 0) AS presence_of_rm_plan,
  
        -- calculation of Citizen Developers count
        COUNT(DISTINCT CASE WHEN len(FSM.API_INTEGRATIONS) > 0 OR len(FSM.API_APPS) > 0 THEN FSM.OWNER_UID END) AS citizen_developers_count,
  
  
        -- Calculate Work App-related KPIs
        COUNT(DISTINCT CASE WHEN POSITION('WorkApps' IN FSM.API_APPS) > 0 THEN FSM.SOLUTION_UID END) AS work_app_count,
        SUM(CASE WHEN POSITION('WorkApps' IN FSM.API_APPS) > 0 THEN FSM.USERS_EXTERNAL_VIEW ELSE 0 END) AS external_views_with_work_app
    FROM 
        FilteredCSM FSM
    JOIN
        AccountsWithMoreThan10Solutions A10 ON FSM.ACCOUNT_ID = A10.ACCOUNT_ID
//    LEFT JOIN 
//        FilteredPlanDaily PD ON FSM.PLAN_UID = PD.PLAN_UID AND FSM.DATE = PD.DATE
    LEFT JOIN 
    ...
    LEFT JOIN 
        PRODUCT.CONTAINER_WORKAPP_J FWA ON 
            FWA.OWNER_PLAN_UID = FSM.PLAN_UID
            AND FSM.DATE BETWEEN FWA.EFFECTIVE_FROM AND FWA.EFFECTIVE_TO
    LEFT JOIN 
