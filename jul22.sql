...
MobileSessions AS (
    SELECT
        s.USER_UID,
        COUNT(DISTINCT s.SESSION_UID) AS mobile_sessions_count
    FROM
        BASE.SESSION s
    WHERE
        LOWER(s.DEVICE) LIKE '%mobile%'
    GROUP BY
        s.USER_UID
),
...
AggregatedData AS (
    SELECT 
        ...
        -- Calculate number of mobile users per account and fiscal month
        COUNT(DISTINCT CASE WHEN ms.mobile_sessions_count > 0 THEN uj.USER_UID END) OVER (PARTITION BY fp.ACCOUNT_ID, sd.YEAR_MONTH) AS mobile_users_count,
...
//        PERCENT_RANK() OVER (ORDER BY MAX(brandfolder_presence) DESC) AS pct_rank_brandfolder_presence
    ...
    LEFT JOIN
        FilteredUserPlans uj ON csm.PLAN_UID = uj.PLAN_UID
    LEFT JOIN
        MobileSessions ms ON uj.USER_UID = ms.USER_UID
//cc
    WHERE
...
SELECT 
    ...
    (SUM(mobile_users_count) / NULLIF(SUM(total_active_users), 0)) * 100 AS pct_mobile_sessions
    PERCENT_RANK() OVER (ORDER BY (SUM(mobile_users_count) / NULLIF(SUM(total_active_users), 0))) AS pct_mobile_sessions
    CASE 
        WHEN pct_mobile_sessions >= 0.90 THEN 7
        WHEN pct_mobile_sessions >= 0.70 AND pct_mobile_sessions < 0.90 THEN 4
        WHEN pct_mobile_sessions >= 0.35 AND pct_mobile_sessions < 0.70 THEN 3
        WHEN pct_mobile_sessions > 0 AND pct_mobile_sessions < 0.35 THEN 1
        ELSE 0
      END AS score_mobile_sessions
